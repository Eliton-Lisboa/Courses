**==ranf.spg  processed by SPAG 4.520  at 11:03 on 18 Oct 1996
...
C----------------------------------------------------------------------C
C  Random nunber generator, fast and rough, machine independent.
...
C  REF: Press, Flannely, Teukolsky, Vetterling, Numerical Recipes (1986)
C----------------------------------------------------------------------C
    IMPLICIT NONE
...
c
    ...
    RANDX = ABS(Iseed*RM)
C    RANDX = Iseed*RM
C    IF (RANDX.LT.0.D+0) THEN
C        STOP '*** Random number is negative ***'
C    END IF
c
    RETURN
    END
...
c
    ...
    RETURN
    END
**==rcarry.spg  processed by SPAG 4.520 at 11:03 on 18 Oct 1996
